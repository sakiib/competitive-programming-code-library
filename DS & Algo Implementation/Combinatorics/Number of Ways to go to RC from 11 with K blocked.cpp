...
void compute( ) {
        dp[1] = Ways( 1 , 1 , a[1].x , a[1].y );
        for( int i = 2; i <= k; i++ ) {
                dp[i] = Ways( 1 , 1 , a[i].x , a[i].y );
                LL subtract = 0;
                for( int j = 1; j < i; j++ ) {
                        subtract += dp[j] * Ways( a[j].x , a[j].y , a[i].x , a[i].y );
                        subtract = subtract % MOD;
                }
                dp[i] -= subtract , dp[i] += MOD , dp[i] %= MOD;
        }
}
int main( int argc , char const *argv[] ) {
        ...
        LL subtract = 0;
        for( int i = 1; i <= k; i++ ) {
                subtract += dp[i] * Ways( a[i].x , a[i].y , r , c );
                subtract %= MOD;
        }
