...
struct segment {
        LL sum , max_suff , max_pref , ans , Bit;
        segment(){}
        segment( LL sum , LL max_suff , LL max_pref , LL ans ,LL Bit ) {
                this -> sum = sum;
...
void Update( int node , int b , int e , int pos , int bit ) {
        if( pos > e || pos < b || b > e ) return;
        if( b == e && b == pos ) {
                Tree[node].Bit  ^= ( 1LL << bit );
                if( __builtin_popcountll( Tree[node].Bit ) == 3 ) {
                        Tree[node].sum = 1;
                        Tree[node].max_pref = 1;
                        Tree[node].max_suff = 1;
                        Tree[node].ans = 1;
                }
                else {
                        Tree[node].sum = -INF;
        ...
        int l = node << 1 , r = l | 1 , m = ( b + e ) >> 1;
        Update( l , b , m , pos , bit );
        Update( r , m + 1 , e , pos , bit );
        Tree[node].sum = Tree[l].sum + Tree[r].sum;
        Tree[node].max_pref = max( Tree[l].max_pref , Tree[l].sum + Tree[r].max_pref );
        Tree[node].max_suff = max( Tree[r].max_suff , Tree[r].sum + Tree[l].max_suff );
        Tree[node].ans = max( Tree[l].ans , Tree[r].ans );
...
segment Query( int node , int b , int e , int i , int j ) {
        if( i > e || j < b || b > e ) return { -INF, -INF, -INF, -INF , -INF };
        if( i <= b && j >= e ) return Tree[node];
        int l = node << 1 , r = l | 1 , m = ( b + e ) >> 1;
        segment s1 = Query( l , b , m , i , j );
        segment s2 = Query( r , m + 1 , e , i , j );
        segment ret = { -INF, -INF, -INF, -INF , -INF };
