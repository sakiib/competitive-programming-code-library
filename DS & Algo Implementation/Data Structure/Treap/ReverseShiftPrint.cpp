...
struct TreapImplicit {
        struct Node {
                ...
                Node( LL _val ) { val = _val; sum = _val; mx = _val; mn = _val; repl = 0; repl_flag = 0;
                                rev = 0; lazy = 0; sz = 1; priority = rand(); l = NULL; r = NULL; par = NULL;
                }
        };
        typedef Node* pnode;
        pnode root;
        ...
        void cyclic_shift( int qL, int qR, LL k, bool left_shift ) {
                if( qL == qR ) return;
                k %= ( qR - qL + 1 );
                pnode l, r, mid, fh, sh;
                split( root, l, r, qL - 1 );
...
int main( int argc , char const *argv[] ) {
        ...
        cin >> n >> q >> m;
        for( int i = 0; i < n; i++ ) cin >> a[i] , treap.insert( i , a[i] );
        while( q-- ) {
                int type;
                cin >> type;
                if( type == 1 ) {
