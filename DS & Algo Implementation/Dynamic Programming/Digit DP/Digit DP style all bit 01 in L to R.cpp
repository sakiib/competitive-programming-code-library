...
/**
maitain single call to be in range of L to R
*/
pll go( int pos , bool bigger , bool smaller ) {
        if( pos < 0 ) return { 0 , 0 };
        ...
        ret = { -1 , INF };
        FOR( i , from , to ) {
                pll temp = go( pos - 1 , bigger | ( i > bitL ) , smaller | ( i < bitR ) );
                temp.first += i;
                if( i ) temp.second += ( 1LL << pos );
